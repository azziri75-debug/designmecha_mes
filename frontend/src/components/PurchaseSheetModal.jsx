...
const PurchaseSheetModal = ({ isOpen, onClose, order, sheetType = 'purchase_order', orderType = 'purchase', onSave }) => {
    ...
    const [metadata, setMetadata] = useState({
        title: activeTab === 'purchase_order' ? "援?留?諛?二??? : "寃?????猶???,
        order_no: "",
        special_notes: "",
        delivery_date: "",
        delivery_place: "(二??붿옄?몃찓移?,
        valid_until: "",
        payment_terms: "?⑺뭹 ???뺢린寃곗젣",
    ...
    const handlePrintWindow = async () => {
        await printAsImage(sheetRef.current, { title: '諛쒖＜??寃ъ쟻?섎ː??, orientation: 'portrait' });
    };
        const generatePDF = async (action = 'save') => {
        if (!sheetRef.current) return;
        setSaving(true);
        try {
            const type = activeTab === 'purchase_order' ? '援щℓ諛쒖＜?? : '寃ъ쟻?섎ː??;
            const vendorName = order.partner?.name || '怨듦툒??;
            const items = order.items || [];
            const firstItemName = items[0]?.product?.name || '?덈챸';
            const extraCount = items.length > 1 ? ` ??${items.length - 1}嫄? : '';
            const date = order.order_date || '?좎쭨';
    ...
    const baseColumns = [
        { key: 'idx', label: '?쒕쾲', subLabel: 'Order', align: 'center' },
        { key: 'name', label: '?덈ぉ', subLabel: 'Description', align: 'left' },
        { key: 'spec', label: '洹쒓꺽', subLabel: 'Gauge', align: 'center' },
        { key: 'qty', label: '?섎웾', subLabel: 'Qty', align: 'center' }
    ];

    const columns = activeTab === 'purchase_order'
        ? [...baseColumns, { key: 'price', label: '?④?', subLabel: 'Unit Price', align: 'right' }, { key: 'total', label: '湲덉븸', subLabel: 'Total Amount', align: 'right' }]
        : [...baseColumns, { key: 'note', label: '鍮꾧퀬', subLabel: 'Remarks', align: 'left' }];
            ...
            <div className="bg-gray-900 w-full max-w-5xl rounded-xl shadow-2xl flex flex-col max-h-[95vh]">
                <div className="flex items-center justify-between p-4 border-b border-gray-700">
                    <div className="flex bg-gray-800 rounded-lg p-0.5 gap-0.5">
                        <button onClick={() => setActiveTab('estimate_request')} className={cn("px-4 py-1.5 rounded-md text-sm font-medium transition-colors", activeTab === 'estimate_request' ? "bg-blue-600 text-white" : "text-gray-400 hover:text-white")}>寃ъ쟻?섎ː??/button>
                        <button onClick={() => setActiveTab('purchase_order')} className={cn("px-4 py-1.5 rounded-md text-sm font-medium transition-colors", activeTab === 'purchase_order' ? "bg-blue-600 text-white" : "text-gray-400 hover:text-white")}>援щℓ諛쒖＜??/button>
                    </div>
