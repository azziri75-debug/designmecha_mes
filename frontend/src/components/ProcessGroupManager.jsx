...
import { Plus, Trash, Pencil, ChevronRight, ChevronDown, Save, X, Settings } from 'lucide-react';
...
const ProcessGroupManager = ({
    ...
    return (
        <div className="bg-gray-800 rounded-xl border border-gray-700 p-6 shadow-sm">
            ...
            {/* Common Processes */}
            <div className="mb-8">
                <h4 className="text-sm font-semibold text-gray-400 mb-3 px-2 flex items-center gap-2">
                    <div className="w-1.5 h-4 bg-emerald-500 rounded-full"></div>
                    공통 공정 (그룹 미지정)
                </h4>
                {commonProcesses.length === 0 ? (
                    <div className="text-sm text-gray-500 px-4 py-3 bg-gray-900 rounded-lg border border-gray-800 text-center">등록된 공통 공정이 없습니다.</div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                        {commonProcesses.map(proc => (
                            <div key={proc.id} className="flex flex-col gap-2 p-3 bg-gray-900/50 border border-emerald-900/30 rounded-lg group hover:border-emerald-700/50 transition-colors">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                        <div className="w-2 h-2 rounded-full bg-emerald-500"></div>
                                        <span className="text-gray-200 font-medium truncate" title={proc.name}>{proc.name}</span>
                                    </div>
                                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button onClick={() => onEditProcess(proc)} className="p-1 text-gray-400 hover:text-white hover:bg-gray-700 rounded transition-colors">
                                            <Pencil className="w-3.5 h-3.5" />
                                        </button>
                                        <button onClick={() => onDeleteProcess(proc.id)} className="p-1 text-red-400 hover:text-red-300 hover:bg-red-400/10 rounded transition-colors">
            ...
            <div className="space-y-4">
                ...
                {majorGroups.length === 0 ? (
                    ...
                    majorGroups.map(major => {
                        ...
                        return (
                            <div key={major.id} className="bg-gray-900 border border-gray-700 rounded-lg overflow-hidden shrink-0">
                                ...
                                <div className="flex items-center justify-between p-4 bg-gray-850 border-b border-gray-800">
                                    ...
                                    <div className="flex gap-2">
                                        <button
                                            ...
                                            className="px-2 py-1 text-xs text-blue-400 hover:bg-blue-400/10 rounded flex items-center gap-1 transition-colors"
                                        >
                                ...
                                <div className="p-3 space-y-3 bg-gray-800/30">
                                    {minors.length === 0 ? (
                                        ...
                                        minors.map(minor => {
                                            ...
                                            return (
                                                <div key={minor.id} className="bg-gray-800 border border-gray-700 rounded-md overflow-hidden">
                                                    <div
                                                    ...
                                                    >
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            {editingNode === minor.id ? (
                                                                ...
                                                                <span className="text-gray-300 font-medium">{minor.name}</span>
                                                            )}
                                                            <span className="text-xs text-gray-500 bg-gray-900 px-2 py-0.5 rounded-full border border-gray-700">{minorProcesses.length} 공정</span>
                                                        </div>
                                                        <div className="flex gap-2">
                                                            <button
                                                                onClick={(e) => {
                                                                ...
                                                                className="px-2 py-1 text-xs text-emerald-400 hover:bg-emerald-400/10 rounded flex items-center gap-1 transition-colors"
                                                            >
                                                                <Plus className="w-3 h-3" /> 공정 추가
                                                            </button>
                                                            <button onClick={(e) => startEditingGroup(minor, e)} className="p-1.5 text-gray-400 hover:text-white hover:bg-gray-700 rounded transition-colors">
                                                                <Pencil className="w-3.5 h-3.5" />
                                                            </button>
                                                            <button onClick={(e) => handleDeleteGroup(minor.id, e)} className="p-1.5 text-red-400 hover:text-red-300 hover:bg-red-400/10 rounded transition-colors">
                                                                <Trash className="w-3.5 h-3.5" />
                                                            </button>
                                                        </div>
                                                    </div>
                                                    ...
                                                    {isMinorExpanded && (
                                                        <div className="border-t border-gray-700 bg-gray-900/50 p-3 pt-0">
                                                            {minorProcesses.length === 0 ? (
                                                                <p className="text-sm text-gray-500 py-3 text-center">등록된 공정이 없습니다.</p>
                                                            ) : (
                                                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 mt-3 pl-8">
                                                                    {minorProcesses.map(proc => (
                                                                        <div key={proc.id} className="flex flex-col gap-1 p-2 bg-gray-800 border border-gray-750 rounded-lg group hover:border-gray-600 transition-colors">
                                                                            <div className="flex items-center justify-between">
                                                                                <div className="flex items-center gap-2 truncate">
                                                                                    <div className="w-1.5 h-1.5 rounded-full bg-emerald-500/50 shrink-0"></div>
                                                                                    <span className="text-gray-300 text-sm truncate" title={proc.name}>{proc.name}</span>
                                                                                </div>
                                                                                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                                                    <button onClick={() => onEditProcess(proc)} className="p-1 text-gray-400 hover:text-white hover:bg-gray-700 rounded transition-colors">
                                                                            ...
                                                                            {proc.course_type && (
                                                                                <div className="pl-3.5">
                                                                                    <span className="text-[9px] px-1.5 py-0.5 rounded bg-gray-900 text-gray-400 border border-gray-700 inline-block">
                                                                                        {proc.course_type === 'INTERNAL' ? '내부' : proc.course_type === 'OUTSOURCING' ? '외주' : '구매'}
                                                                                    </span>
                                                                                </div>
                                                                            )}
