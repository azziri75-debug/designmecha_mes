...
const EstimateModal = ({ isOpen, onClose, onSuccess, partners, estimateToEdit = null }) => {
    // ── Resizable Columns ───────────────────────────────────────
    const EST_LS_KEY = 'estimate_modal_col_widths';
    const EST_DEFAULT = { name: 260, spec: 120, qty: 80, price: 130, amount: 110, note: 120 };
    const [colWidths, setColWidths] = useState(() => {
        try { const s = localStorage.getItem(EST_LS_KEY); if (s) return JSON.parse(s); } catch {}
        return EST_DEFAULT;
    });
    ...
    const onResizerMouseMove = useCallback((e) => {
        ...
        setColWidths(prev => ({ ...prev, [resizingCol.current]: Math.max(50, startWidth.current + e.clientX - startX.current) }));
    }, []);
    const onResizerMouseUp = useCallback(() => {
        ...
        setColWidths(prev => { try { localStorage.setItem(EST_LS_KEY, JSON.stringify(prev)); } catch {} return prev; });
    }, [onResizerMouseMove]);
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <div className="bg-gray-900 rounded-lg overflow-x-auto border border-gray-700">
                            <table className="w-full text-sm text-left text-gray-400" style={{ tableLayout: 'fixed', minWidth: '800px' }}>
                                <colgroup>
                                ...
                                <thead className="bg-gray-800 text-xs uppercase font-medium">
                                    <tr>
                                        {[['name','품명'],['spec','규격'],['qty','수량'],['price','단가 / 통화'],['amount','금액'],['note','비고']].map(([key, label]) => (
                                            <th key={key} className="px-4 py-2" style={{ position: 'relative', userSelect: 'none' }}>
                                                {label}
                                                <div onMouseDown={(e) => onResizerMouseDown(key, e)} style={{ position: 'absolute', right: 0, top: 0, bottom: 0, width: '6px', cursor: 'col-resize', background: 'transparent', zIndex: 1 }} title="드래그하여 열 너비 조정" />
                                            </th>
                                        ))}
