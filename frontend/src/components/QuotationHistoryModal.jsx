...
import { X, Search, ChevronDown, ChevronUp, Check, Calendar } from 'lucide-react';
import api from '../lib/api';
import { cn } from '../lib/utils';

const QuotationHistoryModal = ({ isOpen, onClose, onSelect, partnerId }) => {
    ...
    const filteredEstimates = estimates.filter(est => 
        (est.note || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
        est.items?.some(item => (item.product?.name || '').toLowerCase().includes(searchTerm.toLowerCase()))
    );
            ...
            <div className="bg-gray-800 rounded-xl w-full max-w-5xl max-h-[85vh] flex flex-col shadow-2xl border border-gray-700">
                <div className="p-5 border-b border-gray-700 flex justify-between items-center bg-gray-900/50 rounded-t-xl">
                    <div>
                        ...
                        <p className="text-xs text-gray-400 mt-1">거래처의 과거 견적 내역을 확인하고 현재 문서로 가져옵니다.</p>
                    </div>
                ...
                <div className="p-4 bg-gray-900/30 border-b border-gray-700">
                    <div className="relative">
                        ...
                        <input 
                            type="text"
                            placeholder="품목명 또는 비고로 검색..."
                            value={searchTerm}
                            onChange={(e) => setSearchTerm(e.target.value)}
                            className="w-full bg-gray-800 border-gray-700 rounded-lg pl-10 pr-4 py-2 text-white placeholder:text-gray-500 focus:ring-2 focus:ring-amber-500 outline-none transition-all"
                        />
                    </div>
                </div>
                            ...
                            <table className="w-full text-sm text-left text-gray-300">
                                <thead className="bg-gray-900/80 text-xs font-semibold text-gray-400 uppercase tracking-wider">
                                    <tr>
                                        ...
                                        <th className="px-4 py-3">견적일자</th>
                                ...
                                <tbody className="divide-y divide-gray-700 bg-gray-800/40">
                                    {filteredEstimates.map((est) => {
                                        const representativeItem = est.items?.[0] || {};
                                        const itemCount = est.items?.length || 0;
                                        const itemNameDisplay = representativeItem.product?.name || '품목 정보 없음';
                                        const isExpanded = expandedRows.has(est.id);

                                        return (
                                            <React.Fragment key={est.id}>
                                                <tr className={cn(
                                                    "hover:bg-gray-700/30 transition-colors cursor-pointer",
                                                    isExpanded && "bg-amber-900/10"
                                                )} onClick={() => toggleExpand(est.id)}>
                                                    <td className="px-4 py-4 text-center">
                                                        {isExpanded ? <ChevronUp className="w-4 h-4 text-amber-400" /> : <ChevronDown className="w-4 h-4 text-gray-500" />}
                                                    </td>
                                                    <td className="px-4 py-4 whitespace-nowrap font-medium text-gray-400">{est.estimate_date}</td>
                                                    <td className="px-4 py-4">
                                                        <div className="flex items-center gap-2">
                                                            <span className="text-white hover:text-amber-400 transition-colors">
                                                                {itemNameDisplay}
                                                                {itemCount > 1 && <span className="text-amber-400 text-xs ml-1 font-semibold">외 {itemCount - 1}건</span>}
                                                            </span>
                                                        </div>
                                                    </td>
                                                    ...
                                                    <td className="px-4 py-4 max-w-[150px] truncate text-gray-500" title={est.note}>
                                                        {est.note || '-'}
                                                    </td>
                                                    <td className="px-4 py-4 text-center">
                                                        <button 
                                                            ...
                                                            className="bg-amber-600 hover:bg-amber-500 text-white px-3 py-1.5 rounded-md text-xs font-bold transition-all shadow-lg hover:shadow-amber-500/20 flex items-center gap-1 mx-auto"
                                                        >
                                                    ...
                                                    <tr>
                                                        <td colSpan="8" className="bg-gray-900/50 px-10 py-4 border-t border-gray-700 animate-in slide-in-from-top-2">
                                                            <div className="grid grid-cols-1 gap-2">
                                                                <h4 className="text-xs font-bold text-amber-400 mb-2 uppercase tracking-tighter">견적 상세 정보</h4>
