...
const ApprovalDraftPage = ({ documentData: initialData, onSave, onCancel }) => {
    ...
    return (
        <Box sx={{ bgcolor: '#525659', minHeight: '100vh', py: 4, display: 'flex', flexDirection: 'column', alignItems: 'center', overflow: 'auto' }}>
            ...
            {/* 결재자 의견 표시 (상단, 인쇄 제외) */}
            {documentData?.steps && documentData.steps.some(s => s.comment && s.status === 'APPROVED' && s.comment !== '기안자 자동 승인') && (
                <Box className="no-print" sx={{ width: '100%', maxWidth: '1100px', mb: 3, px: 2 }}>
                    ...
                    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, gap: 2 }}>
                        {documentData.steps.filter(s => s.comment && s.status === 'APPROVED' && s.comment !== '기안자 자동 승인').map((step, idx) => (
                            <Box 
                                key={idx} 
                                sx={{ 
