...
async def sync_plan_item_status(db: AsyncSession, plan_item_id: int):
    ...
    # The requirement says "if 0 then ?€ê¸?PLANNED) OR if there is quantity then ì§„í–‰ì¤?IN_PROGRESS)".
...
async def check_and_complete_production_plan(db: AsyncSession, plan_id: int):
    """
    ëª¨ë“  ê³µì •???„ë£Œ?˜ì—ˆ?”ì? ?•ì¸?˜ê³ , ê·¸ë ‡?¤ë©´ ?ì‚° ê³„íš???„ë£Œ ì²˜ë¦¬?©ë‹ˆ??
    """
    from sqlalchemy.orm import selectinload
    result = await db.execute(
...
async def create_production_plan(
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="?°ì´??ì¤‘ë³µ ?¤ë¥˜: ?™ì¼ ?ˆëª©???€???¬ëŸ¬ ê°œì˜ ?˜ì£¼ ?ëŠ” ë§ˆìŠ¤???°ì´?°ê? ë°œê²¬?˜ì—ˆ?µë‹ˆ?? ê´€ë¦¬ìž?ê²Œ ë¬¸ì˜?˜ì„¸??")
...
async def export_production_plan_excel(
    ...
    if unique_products:
        ...
        if len(unique_products) > 1:
            summary_prod_name += f" ??{len(unique_products) - 1}ê±?
    ...
    ws[f'A{start_row}'] = "?ˆëª…"
    ws[f'D{start_row}'] = "ê·œê²©"
    ws[f'G{start_row}'] = "?¬ì§ˆ"
    ws[f'I{start_row}'] = "?˜ëŸ‰"
    ...
    headers = ["êµ¬ë¶„", "?œë²ˆ", "ê³µì •", "ê³µì •?´ìš©", "?…ì²´", "?ˆëª…", "ê·œê²©", "?˜ëŸ‰", "?œìž‘", "ì¢…ë£Œ"]
    ...
    def get_type_label(ctype):
        if not ctype: return "-"
        if "INTERNAL" in ctype or "?ê?" in ctype: return "?ê?"
        if "OUTSOURCING" in ctype or "?¸ì£¼" in ctype: return "?¸ì£¼"
        if "PURCHASE" in ctype or "êµ¬ë§¤" in ctype: return "êµ¬ë§¤"
        return ctype
...
async def delete_production_plan(
    ...
    for o_item in all_oo_items:
        if o_item.outsourcing_order and o_item.outsourcing_order.status == OutsourcingStatus.COMPLETED:
            raise HTTPException(
                status_code=400, 
                detail="?´ë? ?…ê³  ?„ë£Œ???¸ì£¼ ê±´ì´ ?ˆì–´ ?ì‚°ê³„íš???? œ?????†ìŠµ?ˆë‹¤. ?…ê³ ë¥?ë¨¼ì? ì·¨ì†Œ??ì£¼ì„¸??"
            )
    ...
    try:
        ...
        # Rollback Logic (COMPLETED -> NOT COMPLETED)
            ...
            if plan.stock_production:
                ...
                # 1-3. ?ì‚° ì¤??˜ëŸ‰ ë³µì› (ì§„í–‰ ì¤‘ì¸ ?íƒœë¡?ê°€??ê²½ìš°?ë§Œ ?¤ì‹œ ?ì‚° ì¤‘ìœ¼ë¡??¡ìŒ)
            ...
            for item in plan.items:
                for po_item in item.purchase_items:
                    ...
                    # (User said "?€ê¸??íƒœë¡?ë³€ê²?, which usually means received_quantity = 0 or status = PENDING)
        ...
        raise HTTPException(status_code=400, detail="?°ì´??ì¤‘ë³µ ?¤ë¥˜: ?™ì¼ ?ˆëª©???€???¬ëŸ¬ ê°œì˜ ?¬ê³  ?ˆì½”?œê? ë°œê²¬?˜ì—ˆ?µë‹ˆ?? ê´€ë¦¬ìž?ê²Œ ë¬¸ì˜?˜ì„¸??")
...
async def read_work_logs(
    ...
    # ?¼ë°˜ ?¬ìš©?ì˜ ê²½ìš° ë³¸ì¸???‘ì„±??worker_id)???¼ì?ë§?ì¡°íšŒ ê°€??
...
async def create_work_log(
    ...
    if existing_log and log_in.mode == "CREATE":
        raise HTTPException(status_code=409, detail="?´ë‹¹ ? ì§œ???´ë? ?±ë¡???‘ì—…?¼ì?ê°€ ?ˆìŠµ?ˆë‹¤.")
