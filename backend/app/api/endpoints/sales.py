...
async def export_estimate_excel(
    ...
    try:
        ...
        ws['A4'] = f"?¼ì: {estimate.estimate_date}"
        ws['A4'].alignment = left_align
        
        ws.merge_cells('A5:D5')
        ws['A5'] = f"?˜ì‹ : {estimate.partner.name if estimate.partner else ''} ê·€??
        ws['A5'].alignment = left_align
        ws['A5'].font = bold_font
        
        ws.merge_cells('A6:D6')
        ws['A6'] = f"?©ê³„ê¸ˆì•¡: {estimate.total_amount:,.0f} ??(VAT ë³„ë„)"
        ...
        ws['F6'] = "?±ë¡ë²ˆí˜¸: xxx-xx-xxxxx" # placeholder if actual doesn't exist
    ...
    if order_in.items is not None:
            ...
            if item_in.id and item_in.id in existing_items:
                ...
                if db_item.product_id != item_in.product_id or db_item.quantity != item_in.quantity:
                    ...
                    if plan_check.scalar() > 0:
                        raise HTTPException(
                            status_code=400,
                            detail=f"?´ë? ì§„í–‰ ì¤‘ì¸ ?ì‚° ê³„íš???ˆëŠ” ?ˆëª©??ê·œê²©(?ˆëª©)?´ë‚˜ ?˜ëŸ‰?€ ì§ì ‘ ?˜ì •?????†ìŠµ?ˆë‹¤."
                        )
        ...
        for item_id, item in existing_items.items():
            if item_id not in kept_item_ids:
                # ?š¨ ERP Safeguard: Block deletion if delivery history or work logs exist
                ...
                if hist_check.scalar() > 0:
                    raise HTTPException(
                        status_code=400, 
                        detail=f"?ˆëª©({item.product.name if item.product else item_id})?€(?? ?´ë? ?©í’ˆ ?´ë ¥??ì¡´ì¬?˜ì—¬ ?? œ?????†ìŠµ?ˆë‹¤. ?©í’ˆ ?´ì—­??ë¨¼ì? ì·¨ì†Œ??ì£¼ì„¸??"
                    )
                ...
                if plan_check.scalar() > 0:
                    raise HTTPException(
                        status_code=400, 
                        detail=f"?ˆëª©({item.product.name if item.product else item_id})?€(?? ?´ë? ì§„í–‰ ì¤‘ì¸ ?ì‚° ê³„íš??ì¡´ì¬?˜ì—¬ ?? œ?????†ìŠµ?ˆë‹¤. ê´€???ì‚° ê³„íš??ë¨¼ì? ì·¨ì†Œ??ì£¼ì„¸??"
                    )
...
async def delete_order(
    ...
    if db_order.status == OrderStatus.DELIVERY_COMPLETED or db_order.actual_delivery_date:
        if db_order.status != OrderStatus.PENDING:
            raise HTTPException(status_code=400, detail="?´ë? ?©í’ˆ???„ë£Œ???˜ì£¼ ê±´ì? ?? œ?????†ìŠµ?ˆë‹¤.")
    
    # Cascade delete ?°ê? ?°ì´??(?˜ì¡´ ê´€ê³???ˆœ ë°?ëª…ì‹œ???? œ)
    # 1. ?°ê????ì‚° ê³„íš ID??ê°€?¸ì˜¤ê¸?
    ...
    # [?ˆì „ ?¥ì¹˜] ?´ë? ë°œì£¼/?¸ì£¼ê°€ ì§„í–‰???°ê? ?°ì´?°ê? ?ˆëŠ”ì§€ ?•ì¸ (PENDING ?íƒœ ?œì™¸)
    # 1) ì§ê²°??MRP(?ì¬?Œìš”?? ê¸°ë°˜ ë°œì£¼ ?•ì¸
    ...
    if mrp_po_check.scalars().first():
        raise HTTPException(status_code=400, detail="?´ë? ?ì¬ ë°œì£¼ê°€ ì§„í–‰???˜ì£¼ ê±´ì? ?? œ?????†ìŠµ?ˆë‹¤.")
    ...
    if plan_ids:
        ...
        if item_ids:
            ...
            # 4. ë°œì£¼ ?ˆëª© ?? œ (ProductionPlanItem ì°¸ì¡°)
        ...
        # 7. ë¶ˆëŸ‰ ?´ì—­ ?? œ (ê³„íš ê¸°ì? - ?„ì´?œì´ ?†ëŠ” ê²½ìš° ?€ë¹?
    ...
    # 9. ë¶ˆëŸ‰ ?´ì—­ ?? œ (?˜ì£¼ ê¸°ì? ì§ì ‘ ë§í¬??ê²ƒë“¤)
...
async def create_delivery(
    ...
    """
    ë¶€ë¶??©í’ˆ ì²˜ë¦¬:
    1. SalesOrderItem??delivered_quantity ?…ë°?´íŠ¸
    2. SalesOrder ?íƒœë¥?PARTIALLY_DELIVERED ?ëŠ” DELIVEREDë¡?ë³€ê²?
    3. DeliveryHistory ë°??ˆëª© ?ì„±
    """
    ...
    # [Fix] ?©í’ˆ ?„ë£Œ ???°ê? ?ì‚°ê³„íš ?ë™ ?„ë£Œ ì²˜ë¦¬
    try:
        if all_completed:
...
async def batch_complete_order(
    ...
    """
    ?©í’ˆ ?„ê²° ???„ë°© ê³µì •(?ì‚°ê³„íš, ë°œì£¼) ?¼ê´„ ?„ë£Œ ì²˜ë¦¬
    """
    # 1. ?ì‚° ê³„íš COMPLETED
    ...
    # 2. ê´€???ì‚° ê³µì •(ProductionPlanItem) COMPLETED
    if plan_ids:
    ...
    # 3. ê´€???ì¬ ë°œì£¼(PurchaseOrder) COMPLETED (PENDING/ORDERED ê±?
    # ì§ì ‘ ?°ê²°??PO
    ...
    # 4. ê´€???¸ì£¼ ë°œì£¼(OutsourcingOrder) COMPLETED
    if plan_ids:
...
async def read_delivery_status(
    ...
    """
    ?©í’ˆ ?„í™© ì¡°íšŒë¥??„í•œ ?˜ì£¼ ëª©ë¡ (ë°°ì†¡ ?´ë ¥ ?¬í•¨)
    """
    query = select(SalesOrder).options(
...
# ?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€
# ê±°ë˜ëª…ì„¸??PDF ì²¨ë? (?©í’ˆ ?´ë ¥ ??statement_json ?…ë°?´íŠ¸)
# ?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€
...
async def attach_statement_to_delivery_history(
    ...
    """ê±°ë˜ëª…ì„¸??PDFë¥??€?¥í•˜ê³??´ë‹¹ DeliveryHistory??ë§í¬ë¥??€?¥í•©?ˆë‹¤."""
...
async def update_delivery_history(
    ...
    """?©í’ˆ ?´ë ¥ ?˜ì • API (?˜ëŸ‰ ë³€ê²????˜ì£¼ ?”ëŸ‰ ë°??íƒœ ?™ê¸°???¬í•¨)"""
    ...
    if delivery_update.items is not None:
        for item_in in delivery_update.items:
            ...
            if not dh_item:
                continue # ?¹ì? ?ˆë¡œ ì¶”ê??˜ëŠ” ë¡œì§???„ìš”?????ˆìœ¼???¬ê¸°???˜ì •ë§?ì²˜ë¦¬
...
# ?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€
# ?©í’ˆ ?´ë ¥ ?? œ/ì·¨ì†Œ (DELETE) - ?˜ì£¼ ?”ëŸ‰ ë³µì› ?¬í•¨
# ?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€
...
async def delete_delivery_history(
    ...
    # 1) ?©í’ˆ???˜ëŸ‰???˜ì£¼ ?ˆëª©??delivered_quantity?ì„œ ì°¨ê° (?ë³µ)
...
async def fix_mismatched_plans_in_db(db: AsyncSession = Depends(deps.get_db)):
    ...
    try:
        ...
        await db.commit()
        return {"status": "success", "message": "?´ê¸‹???ì‚°ê³„íš ?ˆëª© ë°??˜ëŸ‰ ?°ì´?°ê? ?˜ì£¼ ?°ì´?°ì— ë§ì¶° ?•ìƒ?ìœ¼ë¡??™ê¸°?”ë˜?ˆìŠµ?ˆë‹¤. ?ˆë¡œê³ ì¹¨??ì£¼ì„¸??"}
    except Exception as e:
        await db.rollback()
        raise HTTPException(status_code=500, detail=str(e))
