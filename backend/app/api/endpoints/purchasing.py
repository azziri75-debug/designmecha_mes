...
async def get_unordered_requirements(
    ...
    """
    Í∏∞Î°ù??ÎØ∏Î∞úÏ£??åÏöî??MRP) Î¶¨Ïä§??Ï°∞Ìöå API
    """
    """
    Í∏∞Î°ù??ÎØ∏Î∞úÏ£??åÏöî??MRP) Î¶¨Ïä§??Ï°∞Ìöå API
    """
    ...
    for req in requirements:
        ...
        if req.plan:
            if req.plan.order:
                req.linkage_info = f"?ùÏÇ∞Í≥ÑÌöç({req.plan.order.order_no})"
                req.sales_order_number = req.plan.order.order_no
            elif req.plan.stock_production:
                req.linkage_info = f"?ùÏÇ∞Í≥ÑÌöç({req.plan.stock_production.production_no})"
            else:
                req.linkage_info = f"?ùÏÇ∞Í≥ÑÌöç(ID:{req.plan.id})"
        elif req.order:
            req.linkage_info = f"?òÏ£º({req.order.order_no})"
...
async def get_consumable_waits(
    ...
    """
    Í≤∞Ïû¨ ?πÏù∏???åÎ™®??Î∞úÏ£º ?ÄÍ∏∞Ïó¥ Ï°∞Ìöå
    """
...
async def register_consumable_order(
    ...
    """?åÎ™®??Î∞úÏ£º ?±Î°ù (Í∏∞Ï°¥ Îß§Ìïë or ?†Í∑ú ?±Î°ù ??Î∞úÏ£º??Î∞úÌñâ)"""
    # 1. Get wait item
    ...
    if not wait_item:
        raise HTTPException(status_code=404, detail="?ÄÍ∏???™©??Ï∞æÏùÑ ???ÜÏäµ?àÎã§.")
    if wait_item.status != "PENDING":
        raise HTTPException(status_code=400, detail="?¥Î? Ï≤òÎ¶¨????™©?ÖÎãà??")
        
    # 2. Handle Product
    product_id = req.product_id
    if not product_id:
        if not req.new_product_name:
            raise HTTPException(status_code=400, detail="Í∏∞Ï°¥ ?àÎ™©???†ÌÉù?òÍ±∞???†Í∑ú ?àÎ™©Î™ÖÏùÑ ?ÖÎ†•?¥Ïïº ?©Îãà??")
        ...
        new_product = Product(
            ...
            note=f"?åÎ™®?àÎ∞úÏ£ºÎì±Î°??ÄÍ∏∞ID:{req.wait_id})"
        )
        db.add(new_product)
        await db.flush()
    ...
    await db.commit()
    return {"message": "Î∞úÏ£ºÍ∞Ä ?±Í≥µ?ÅÏúºÎ°??±Î°ù?òÏóà?µÎãà??", "id": new_po.id, "order_no": order_no}



    # 2. Ï¥??åÏöî??Demand) Í≥ÑÏÇ∞ (Î∂Ä?àÎ≥Ñ ?©Í≥Ñ)
    ...
    for so in sales_orders:
        for item in so.items:
            ...
            if not bom_items:
                # BOM???ÜÏúºÎ©??ÑÏ†ú???êÏ≤¥Í∞Ä ?êÏûê??Î∂Ä?àÏù∏ Í≤ΩÏö∞?¥Í±∞?? ?†Ï†ú?àÏù¥???ÑÎùΩ??Í≤ΩÏö∞
                # ?îÍµ¨?¨Ìï≠???∞Îùº BOM???ÜÏúºÎ©??òÏúÑ ?åÏöî??0 Ï≤òÎ¶¨ (?? ?ÑÏ†ú???êÏ≤¥Í∞Ä PART??Í≤ΩÏö∞???àÏùÑ ???àÏùå)
                # ?¨Í∏∞?úÎäî ?ÑÏ†ú???êÏ≤¥???åÏöî?âÏóê ?£Ï? ?äÍ≥†, ?§ÏßÅ 'Î∂Ä???êÏûê?? ?àÎ≤®???åÏöî?âÎßå ?∞Ï∂ú??
                # ÎßåÏïΩ ?ÑÏ†ú?àÏù¥ RAW_MATERIAL?¥ÎÇò PART?ºÎ©¥ ?îÍµ¨?¨Ìï≠???¨Ìï®?????àÏùå
    ...
    for pid, demand in total_demand.items():
        ...
        # 3-2. ÎØ∏ÏûÖÍ≥?Î∞úÏ£º ?îÎüâ(Open Purchase Qty) Ï°∞Ìöå
        # PENDING, ORDERED, PARTIAL ?ÅÌÉú??PO ?ÑÏù¥?úÎì§ Ï§?(?òÎüâ - ?ÖÍ≥†?òÎüâ) ?©Í≥Ñ
        ...
        # 3-3. ÏµúÏ¢Ö ?ÑÏöî ?òÎüâ Í≥ÑÏÇ∞ Í≥µÏãù: (Ï¥??åÏöî?? - (?ÑÏû¨Í≥? - (Î∞úÏ£º ?îÎüâ)
...
async def read_pending_purchase_items(
    ...
    query = select(ProductionPlanItem).join(ProductionPlanItem.plan)\
        ...
        .where(or_(
            ...
            ProductionPlanItem.course_type.like('%Íµ¨Îß§%')
        ))\
        .where(
...
async def read_pending_outsourcing_items(
    ...
    query = select(ProductionPlanItem).join(ProductionPlanItem.plan)\
        ...
        .where(or_(
            ...
            ProductionPlanItem.course_type.like('%?∏Ï£º%')
        ))\
        .where(
...
async def create_purchase_order(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Î∞úÏ£º ?±Î°ù Ï§??§Î•ò Î∞úÏÉù (DB ?úÏïΩÏ°∞Í±¥ ?πÏ? ?§ÌÇ§Îß?Ï∂©Îèå): {str(e)}")
...
async def complete_purchase_order(
    ...
    """Íµ¨Îß§ Î∞úÏ£º???ÑÎ£å Ï≤òÎ¶¨ (?ÖÍ≥† Î∞òÏòÅ)"""
    query = select(PurchaseOrder).options(selectinload(PurchaseOrder.items)).where(PurchaseOrder.id == order_id)
    res = await db.execute(query)
    order = res.scalars().first()
    
    if not order:
        raise HTTPException(status_code=404, detail="Î∞úÏ£º?úÎ? Ï∞æÏùÑ ???ÜÏäµ?àÎã§.")
    if order.status == PurchaseStatus.COMPLETED:
        raise HTTPException(status_code=400, detail="?¥Î? ?ÑÎ£å(?ÖÍ≥†)??Î∞úÏ£ºÍ±¥ÏûÖ?àÎã§.")
    ...
    await db.commit()
    return {"message": "Î∞úÏ£º ?ÑÎ£å Î∞??ïÏÉÅ ?ÖÍ≥† Ï≤òÎ¶¨?òÏóà?µÎãà??", "order_no": order.order_no}
...
async def trigger_fix_consumable_types(
    ...
    try:
        ...
        return {
            "status": "success", 
            "message": f"Ï¥?{updated_count}Í±¥Ïùò Î∞úÏ£º???†Ìòï??'?åÎ™®??CONSUMABLE)'?ºÎ°ú Î≥¥Ï†ï?òÏóà?µÎãà??",
...
async def read_outsourcing_orders(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"?∏Ï£ºÎ∞úÏ£º Ï°∞Ìöå Ï§??§Î•ò Î∞úÏÉù: {str(e)}")
...
async def delete_material_requirement(
    ...
    """
    ÎØ∏Î∞úÏ£??åÏöî??MRP) ?àÏΩî??Í∞úÎ≥Ñ ??†ú.
    ?? ?¥Î? Î∞úÏ£º(ORDERED) ?ÅÌÉú?¥Í±∞??Î∞úÏ£º ?àÎ™©???∞Í≤∞??Í≤ΩÏö∞ ??†ú Î∂àÍ?.
    """
    req = await db.get(MaterialRequirement, requirement_id)
    if not req:
        raise HTTPException(status_code=404, detail="?åÏöî???∞Ïù¥?∞Î? Ï∞æÏùÑ ???ÜÏäµ?àÎã§.")

    # 1. ?ÅÌÉú Ï≤¥ÌÅ¨
    if req.status == "ORDERED":
         raise HTTPException(
             status_code=400, 
             detail="?¥Î? Î∞úÏ£ºÍ∞Ä ?ÑÎ£å???åÏöî???∞Ïù¥?∞Îäî ??†ú?????ÜÏäµ?àÎã§. Î∞úÏ£ºÎ•?Î®ºÏ? Ï∑®ÏÜå??Ï£ºÏÑ∏??"
         )

    # 2. Î∞úÏ£º ?àÎ™© ?∞Í≤∞ Ï≤¥ÌÅ¨ (Î∞©Ïñ¥???ïÏù∏)
    stmt = select(PurchaseOrderItem).where(PurchaseOrderItem.material_requirement_id == requirement_id)
    result = await db.execute(stmt)
    if result.scalars().first():
        raise HTTPException(
            status_code=400,
            detail="???åÏöî?âÍ≥º ?∞Í≤∞??Î∞úÏ£º ?àÎ™©??Ï°¥Ïû¨?©Îãà?? Î∞úÏ£º ?àÎ™©??Î®ºÏ? ??†ú??Ï£ºÏÑ∏??"
        )

    await db.delete(req)
    await db.commit()
    return {"message": "?ïÏÉÅ?ÅÏúºÎ°???†ú?òÏóà?µÎãà??"}
