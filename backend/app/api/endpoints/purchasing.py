...
async def get_unordered_requirements(
    ...
    """
    æ¹²ê³•ì¤??èª˜ëªƒì»»äºŒ????Š‚??MRP) ?±ÑŠë’ª??è­°ê³ ??API
    """
    """
    æ¹²ê³•ì¤??èª˜ëªƒì»»äºŒ????Š‚??MRP) ?±ÑŠë’ª??è­°ê³ ??API
    """
    ...
    for req in requirements:
        ...
        if req.plan:
            if req.plan.order:
                req.linkage_info = f"??¹ê¶›?¨ê¾ª??{req.plan.order.order_no})"
                req.sales_order_number = req.plan.order.order_no
            elif req.plan.stock_production:
                req.linkage_info = f"??¹ê¶›?¨ê¾ª??{req.plan.stock_production.production_no})"
            else:
                req.linkage_info = f"??¹ê¶›?¨ê¾ª??ID:{req.plan.id})"
        elif req.order:
            req.linkage_info = f"??ï¼œ({req.order.order_no})"
...
async def register_consumable_order(
    ...
    """???ˆ??è«›ì’–ï¼??ê¹…ì¤‰ (æ¹²ê³—??ï§ã…½ë¸?or ?ì¢‰í‡‹ ?ê¹…ì¤‰ ??è«›ì’–ï¼??è«›ì’—ë»?"""
    # 1. Get wait item
    ...
    if not wait_item:
        raise HTTPException(status_code=404, detail="??æ¹??????ï§¡ì– ??????ë’¿??ˆë–.")
    if wait_item.status != "PENDING":
        raise HTTPException(status_code=400, detail="??€? ï§£ì„????????…ë•²??")
        
    # 2. Handle Product
    product_id = req.product_id
    if not product_id:
        if not req.new_product_name:
            raise HTTPException(status_code=400, detail="æ¹²ê³—????ˆã‰???ì¢ê¹®??êµ…???ì¢‰í‡‹ ??ˆã‰ï§ë‚†????…ì °??ë¹ ??¸ë•²??")
        ...
        new_product = Product(
            ...
            note=f"???ˆ??ˆì»»äºŒì‡°ë²‘æ¿¡???æ¹²ìº§D:{req.wait_id})"
        )
        db.add(new_product)
        await db.flush()
    ...
    await db.commit()
    return {"message": "è«›ì’–ï¼œåª›? ?ê¹ƒë‚¬?ê³¸ì‘æ¿??ê¹…ì¤‰??ë???¬ë•²??", "id": new_po.id, "order_no": order_no}



    # 2. ?????Š‚??Demand) ?¨ê¾©ê¶?(?ºÂ€??ˆí€???·í€?
    ...
    for so in sales_orders:
        for item in so.items:
            ...
            if not bom_items:
                # BOM????ì‘ï§??ê¾©ì £???ë¨?»œåª›Â€ ?ë¨?˜„???ºÂ€??‰ì”¤ å¯ƒìŒ???¿êµ…?? ?ì¢ì £??‰ì” ???ê¾¨ì”«??å¯ƒìŒ??
                # ?ë¶½ë„??ë¹???ê³•ì”ª BOM????ì‘ï§???ì ???Š‚??0 ï§£ì„??(?? ?ê¾©ì £???ë¨?»œåª›Â€ PART??å¯ƒìŒ?????‰ì“£ ????‰ì“¬)
                # ??ë¦??•ë’— ?ê¾©ì £???ë¨?»œ?????Š‚??±ë¿‰ ?ï½? ??„í€? ??¼ì­… '?ºÂ€???ë¨?˜„?? ??ˆêº¼?????Š‚??°ì­” ?ê³—í…§??
                # ï§ëš¯ë¹??ê¾©ì £??‰ì”  RAW_MATERIAL??€êµ?PART??°ãˆƒ ?ë¶½ë„??ë¹????ë¸??????‰ì“¬
    ...
    # 3. åª??ºÂ€??ˆí€??ê¾©ì˜±??è«?èª˜ëª„??€?è«›ì’–ï¼??ë¶¾ì›¾ ?ê³—í…§??ë¿¬ ï§¤ì’–ì¥??ê¾©ìŠ‚ ??ì›¾ ?ê¾©í…§
    mrp_results = []
    ...
    for pid, demand in total_demand.items():
        ...
        # 3-2. èª˜ëª„??€?è«›ì’–ï¼??ë¶¾ì›¾(Open Purchase Qty) è­°ê³ ??
        # PENDING, ORDERED, PARTIAL ?ê³¹ê¹­??PO ?ê¾©ì” ??•ë±¾ ä»?(??ì›¾ - ??ƒí€??ì›¾) ??·í€?
        ...
        # 3-3. ï§¤ì’–ì¥??ê¾©ìŠ‚ ??ì›¾ ?¨ê¾©ê¶??¨ë“­?? (?????Š‚?? - (?ê¾©ì˜±?? - (è«›ì’–ï¼??ë¶¾ì›¾)
...
async def read_pending_purchase_items(
    ...
    query = select(ProductionPlanItem).join(ProductionPlanItem.plan)\
        ...
        .where(or_(
            ...
            ProductionPlanItem.course_type.like('%?´Ñ‰â„“%')
        ))\
        .where(
...
async def read_pending_outsourcing_items(
    ...
    query = select(ProductionPlanItem).join(ProductionPlanItem.plan)\
        ...
        .where(or_(
            ...
            ProductionPlanItem.course_type.like('%?ëª„ï¼œ%')
        ))\
        .where(
...
async def create_purchase_order(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"è«›ì’–ï¼??ê¹…ì¤‰ ä»???»ìªŸ è«›ì’–ê¹?(DB ??–ë¹Ÿè­°ê³Œêµ??ë±€? ??½ê¶ï§??°â‘¸ë£?: {str(e)}")
...
async def complete_purchase_order(
    ...
    """?´Ñ‰â„“ è«›ì’–ï¼???ê¾¨ì¦º ï§£ì„??(??ƒí€?è«›ì„??"""
    query = select(PurchaseOrder).options(selectinload(PurchaseOrder.items)).where(PurchaseOrder.id == order_id)
    res = await db.execute(query)
    order = res.scalars().first()
    
    if not order:
        raise HTTPException(status_code=404, detail="è«›ì’–ï¼??? ï§¡ì– ??????ë’¿??ˆë–.")
    if order.status == PurchaseStatus.COMPLETED:
        raise HTTPException(status_code=400, detail="??€? ?ê¾¨ì¦º(??ƒí€???è«›ì’–ï¼œå«„?ì—¯??ˆë–.")
    ...
    await db.commit()
    return {"message": "è«›ì’–ï¼??ê¾¨ì¦º è«??ëº¤ê¸½ ??ƒí€?ï§£ì„???ë???¬ë•²??", "order_no": order.order_no}
...
async def trigger_fix_consumable_types(
    ...
    try:
        ...
        return {
            "status": "success", 
            "message": f"??{updated_count}å«„ëŒ??è«›ì’–ï¼???ì¢ì‚??'???ˆ??CONSUMABLE)'??°ì¤ˆ è¹‚ëŒ???ë???¬ë•²??",
...
async def read_outsourcing_orders(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"?ëª„ï¼œè«›ì’–ï¼?è­°ê³ ??ä»???»ìªŸ è«›ì’–ê¹? {str(e)}")
...
async def delete_material_requirement(
    ...
    """
    èª˜ëªƒì»»äºŒ????Š‚??MRP) ??‰í«??åª›ì’•??????
    ?? ??€? è«›ì’–ï¼?ORDERED) ?ê³¹ê¹­??¿êµ…??è«›ì’–ï¼???ˆã‰???ê³Œê»??å¯ƒìŒ???????ºë‡?.
    """
    req = await db.get(MaterialRequirement, requirement_id)
    if not req:
        raise HTTPException(status_code=404, detail="???Š‚???ê³—ì” ?ê³? ï§¡ì– ??????ë’¿??ˆë–.")

    # 1. ?ê³¹ê¹­ ï§£ëŒ„ê²?
    if req.status == "ORDERED":
         raise HTTPException(
             status_code=400, 
             detail="??€? è«›ì’–ï¼œåª›? ?ê¾¨ì¦º?????Š‚???ê³—ì” ?ê³•ë’— ?????????ë’¿??ˆë–. è«›ì’–ï¼œç‘œ??’ì‡±? ?â‘¥???äºŒì‡±ê½??"
         )

    # 2. è«›ì’–ï¼???ˆã‰ ?ê³Œê» ï§£ëŒ„ê²?(è«›â‘¹ë¼???ëº¤ì”¤)
    stmt = select(PurchaseOrderItem).where(PurchaseOrderItem.material_requirement_id == requirement_id)
    result = await db.execute(stmt)
    if result.scalars().first():
        raise HTTPException(
            status_code=400,
            detail="?????Š‚???‚µ ?ê³Œê»??è«›ì’–ï¼???ˆã‰??è­°ëŒ???¸ë•²?? è«›ì’–ï¼???ˆã‰???’ì‡±? ?????äºŒì‡±ê½??"
        )

    await db.delete(req)
    await db.commit()
    return {"message": "?ëº¤ê¸½?ê³¸ì‘æ¿??????ë???¬ë•²??"}
