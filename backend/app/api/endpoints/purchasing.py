...
async def get_unordered_requirements(
    ...
    for req in requirements:
        ...
        if req.plan:
            if req.plan.order:
                req.linkage_info = f"?앹궛怨꾪쉷({req.plan.order.order_no})"
                req.sales_order_number = req.plan.order.order_no
            elif req.plan.stock_production:
                req.linkage_info = f"?앹궛怨꾪쉷({req.plan.stock_production.production_no})"
            else:
                req.linkage_info = f"?앹궛怨꾪쉷(ID:{req.plan.id})"
        elif req.order:
            req.linkage_info = f"?섏＜({req.order.order_no})"
...
async def register_consumable_order(
    ...
    """?뚮え??諛쒖＜ ?깅줉 (湲곗〈 留ㅽ븨 or ?좉퇋 ?깅줉 ??諛쒖＜??諛쒗뻾)"""
    # 1. Get wait item
    ...
    if not wait_item:
        raise HTTPException(status_code=404, detail="?湲???ぉ??李얠쓣 ???놁뒿?덈떎.")
    if wait_item.status != "PENDING":
        raise HTTPException(status_code=400, detail="?대? 泥섎━????ぉ?낅땲??")
        
    # 2. Handle Product
    product_id = req.product_id
    if not product_id:
        if not req.new_product_name:
            raise HTTPException(status_code=400, detail="湲곗〈 ?덈ぉ???좏깮?섍굅???좉퇋 ?덈ぉ紐낆쓣 ?낅젰?댁빞 ?⑸땲??")
        ...
        new_product = Product(
            ...
            note=f"?뚮え?덈컻二쇰벑濡??湲캧D:{req.wait_id})"
        )
        db.add(new_product)
        await db.flush()
    ...
    await db.commit()
    return {"message": "諛쒖＜媛 ?깃났?곸쑝濡??깅줉?섏뿀?듬땲??", "id": new_po.id, "order_no": order_no}



    # 2. 珥??뚯슂??Demand) 怨꾩궛 (遺?덈퀎 ?⑷퀎)
    ...
    for so in sales_orders:
        for item in so.items:
            ...
            if not bom_items:
                # BOM???놁쑝硫??꾩젣???먯껜媛 ?먯옄??遺?덉씤 寃쎌슦?닿굅?? ?좎젣?덉씠???꾨씫??寃쎌슦
                # ?붽뎄?ы빆???곕씪 BOM???놁쑝硫??섏쐞 ?뚯슂??0 泥섎━ (?? ?꾩젣???먯껜媛 PART??寃쎌슦???덉쓣 ???덉쓬)
                # ?ш린?쒕뒗 ?꾩젣???먯껜???뚯슂?됱뿉 ?ｌ? ?딄퀬, ?ㅼ쭅 '遺???먯옄?? ?덈꺼???뚯슂?됰쭔 ?곗텧??
                # 留뚯빟 ?꾩젣?덉씠 RAW_MATERIAL?대굹 PART?쇰㈃ ?붽뎄?ы빆???ы븿?????덉쓬
    ...
    # 3. 媛?遺?덈퀎 ?꾩옱怨?諛?誘몄엯怨?諛쒖＜ ?붾웾 ?곗텧?섏뿬 理쒖쥌 ?꾩슂 ?섎웾 ?꾩텧
    mrp_results = []
    ...
    for pid, demand in total_demand.items():
        ...
        # 3-2. 誘몄엯怨?諛쒖＜ ?붾웾(Open Purchase Qty) 議고쉶
        # PENDING, ORDERED, PARTIAL ?곹깭??PO ?꾩씠?쒕뱾 以?(?섎웾 - ?낃퀬?섎웾) ?⑷퀎
        ...
        # 3-3. 理쒖쥌 ?꾩슂 ?섎웾 怨꾩궛 怨듭떇: (珥??뚯슂?? - (?꾩옱怨? - (諛쒖＜ ?붾웾)
...
async def read_pending_purchase_items(
    ...
    query = select(ProductionPlanItem).join(ProductionPlanItem.plan)\
        ...
        .where(or_(
            ...
            ProductionPlanItem.course_type.like('%援щℓ%')
        ))\
        .where(
...
async def read_pending_outsourcing_items(
    ...
    query = select(ProductionPlanItem).join(ProductionPlanItem.plan)\
        ...
        .where(or_(
            ...
            ProductionPlanItem.course_type.like('%?몄＜%')
        ))\
        .where(
...
async def create_purchase_order(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"諛쒖＜ ?깅줉 以??ㅻ쪟 諛쒖깮 (DB ?쒖빟議곌굔 ?뱀? ?ㅽ궎留?異⑸룎): {str(e)}")
...
async def complete_purchase_order(
    ...
    if not order:
        raise HTTPException(status_code=404, detail="諛쒖＜?쒕? 李얠쓣 ???놁뒿?덈떎.")
    if order.status == PurchaseStatus.COMPLETED:
        raise HTTPException(status_code=400, detail="?대? ?꾨즺(?낃퀬)??諛쒖＜嫄댁엯?덈떎.")
    ...
    await db.commit()
    return {"message": "諛쒖＜ ?꾨즺 諛??뺤긽 ?낃퀬 泥섎━?섏뿀?듬땲??", "order_no": order.order_no}
...
async def trigger_fix_consumable_types(
    ...
    try:
        ...
        return {
            "status": "success", 
            "message": f"珥?{updated_count}嫄댁쓽 諛쒖＜???좏삎??'?뚮え??CONSUMABLE)'?쇰줈 蹂댁젙?섏뿀?듬땲??",
...
async def read_outsourcing_orders(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"?몄＜諛쒖＜ 議고쉶 以??ㅻ쪟 諛쒖깮: {str(e)}")
...
async def delete_material_requirement(
    ...
    """
    誘몃컻二??뚯슂??MRP) ?덉퐫??媛쒕퀎 ??젣.
    ?? ?대? 諛쒖＜(ORDERED) ?곹깭?닿굅??諛쒖＜ ?덈ぉ???곌껐??寃쎌슦 ??젣 遺덇?.
    """
    req = await db.get(MaterialRequirement, requirement_id)
    if not req:
        raise HTTPException(status_code=404, detail="?뚯슂???곗씠?곕? 李얠쓣 ???놁뒿?덈떎.")

    # 1. ?곹깭 泥댄겕
    if req.status == "ORDERED":
         raise HTTPException(
             status_code=400, 
             detail="?대? 諛쒖＜媛 ?꾨즺???뚯슂???곗씠?곕뒗 ??젣?????놁뒿?덈떎. 諛쒖＜瑜?癒쇱? 痍⑥냼??二쇱꽭??"
         )

    # 2. 諛쒖＜ ?덈ぉ ?곌껐 泥댄겕 (諛⑹뼱???뺤씤)
    stmt = select(PurchaseOrderItem).where(PurchaseOrderItem.material_requirement_id == requirement_id)
    result = await db.execute(stmt)
    if result.scalars().first():
        raise HTTPException(
            status_code=400,
            detail="???뚯슂?됯낵 ?곌껐??諛쒖＜ ?덈ぉ??議댁옱?⑸땲?? 諛쒖＜ ?덈ぉ??癒쇱? ??젣??二쇱꽭??"
        )

    await db.delete(req)
    await db.commit()
    return {"message": "?뺤긽?곸쑝濡???젣?섏뿀?듬땲??"}
