...
async def quick_create_process(
    ...
    """
    ?ÑÎ°†?∏Ïóî???úÌíà ?òÏ†ï ?îÎ©¥?êÏÑú Ï¶âÏãú ??Í≥µÏ†ï???±Î°ù?òÍ∏∞ ?ÑÌïú API
    """
    new_process = Process(
        name=process_in.name,
        course_type=process_in.course_type,
        group_id=process_in.group_id,
        # major_group_id??ProductGroup ?åÏù¥Î∏îÏóê??Î≥ÑÎèÑ ?ÑÎìúÍ∞Ä ?àÏúº??Process ?åÏù¥Î∏îÏóê??group_id(?åÍ∑∏Î£?Îß??∞Í≤∞?òÏñ¥ ?àÏñ¥
        # ?ÑÏöî??Process Î™®Îç∏???ïÏù∏?¥Ïïº ?òÏ?Îß??ÑÏû¨ ?§ÌÇ§ÎßàÏÉÅ group_id(minor)Îß?Î∞õÏùå.
    )
    db.add(new_process)
    await db.commit()
    await db.refresh(new_process)
...
async def create_product(
    ...
    new_stock = Stock(product_id=new_product.id, current_quantity=0, location="Í∏∞Î≥∏Ï∞ΩÍ≥†")
    db.add(new_stock)
...
async def read_products(
    ...
    if item_type:
        # ÏßÄ?êÌïò??Í≤ΩÏö∞ ÏΩ§ÎßàÎ°?Íµ¨Î∂Ñ???¨Îü¨ ?Ä?ÖÏùÑ Î∞õÏùÑ ???àÎèÑÎ°?Ï≤òÎ¶¨
        if "," in item_type:
...
async def get_product_purchase_history(
    ...
    """
    ?πÏ†ï ?úÌíà??Í≥ºÍ±∞ Íµ¨Îß§(Î∞úÏ£º) ?¥Ïó≠ Ï°∞Ìöå
    """
...
async def get_product_price_history(
    ...
    """
    ?πÏ†ï ?úÌíà??Í≥ºÍ±∞ Í≤¨Ï†Å Î∞??òÏ£º ?¥Ïó≠ ?µÌï© Ï°∞Ìöå
    """
    history = []
...
async def get_process_cost_history(
    ...
    """
    ?πÏ†ï ?úÌíà-Í≥µÏ†ï Ï°∞Ìï©???êÍ? ?¥Î†• (Íµ¨Îß§/?∏Ï£º) Ï°∞Ìöå
    """
    # Check process type
...
async def get_bom(
    ...
    """
    ?πÏ†ï ?úÌíà??BOM(?òÏúÑ Î∂Ä??Î™©Î°ù) Ï°∞Ìöå
    """
    result = await db.execute(
        select(BOM)
...
async def update_bom(
    ...
    """
    ?πÏ†ï ?úÌíà??BOM ?ÑÏ≤¥ ÍµêÏ≤¥ (?Ä??Î≤ÑÌäº)
    """
    try:
        # Í∏∞Ï°¥ BOM ?ÑÏ≤¥ ??†ú
        ...
        for item in items:
            if item.child_product_id == product_id:
                raise HTTPException(status_code=400, detail="?êÍ∏∞ ?êÏã†??BOM ?òÏúÑ ?àÎ™©?ºÎ°ú ?§Ï†ï?????ÜÏäµ?àÎã§.")
            bom_row = BOM(
        ...
        await db.rollback()
        raise HTTPException(status_code=500, detail=f"BOM ?Ä???§Ìå®: {str(e)}")
...
async def delete_bom_item(
    ...
    if not bom_item:
        raise HTTPException(status_code=404, detail="BOM ??™©??Ï∞æÏùÑ ???ÜÏäµ?àÎã§.")
    ...
    try:
        ...
        await db.rollback()
        raise HTTPException(status_code=500, detail=f"BOM ??†ú ?§Ìå®: {str(e)}")
